...
class QRCodeGenerator extends CWidget {
    ...
    public function init()
    {
        ...
        if (!$this->filePath){
            $this->filePath = realpath(Yii::app()->getBasePath().'/../upload');
        }
        ...
        if (!isset($this->fileUrl)){
            $this->fileUrl = Yii::app()->baseUrl . '/upload';
        }
	...
	private function sanitizeFileName($string, $force_lowercase = true, $anal = false) {
        $strip = array("~", "`", "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "_", "=", "+", "[", "{", "]",
            "}", "\\", "|", ";", ":", "\"", "'", "&#8216;", "&#8217;", "&#8220;", "&#8221;", "&#8211;", "&#8212;",
            "â€”", "â€“", ",", "<", ".", ">", "/", "?");
